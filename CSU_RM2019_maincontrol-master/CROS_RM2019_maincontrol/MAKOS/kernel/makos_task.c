...
p_TCB	task_create_full(task_f func,u8 * pre_tcb_name,void* param, task_prio_t prio, U32 print_size)
{
	p_TCB		ptcb;
	
	ptcb			= (p_TCB)mymalloc(SRAMIN,sizeof(TCB_t));
	ptcb->pr_tcb_name = pre_tcb_name;
	ptcb->sp		= &ptcb->task_stack[TASK_STACK_SIZE-1]+1;
	...
	*(--ptcb->sp)		= (U32)0x04040404;	//R4
	...
	printf("stack's end address is:%x\r\n",(U32)&ptcb->task_stack[TASK_STACK_SIZE]);
