...
void get_stack(p_TCB p_tcp,p_stack_info_type stack_info)
{
	u32 remain_stack = 0;
	U32 * p_stack;
	U32 stack_total = TASK_STACK_SIZE;
	p_stack = &p_tcp->task_stack[TASK_STACK_SIZE-1]+1;
	U32 zero_num = 0;
	...
	stack_info->remain_stack = remain_stack;
	stack_info->use_stack = TASK_STACK_SIZE - remain_stack;
	stack_info->all_stack = TASK_STACK_SIZE;
